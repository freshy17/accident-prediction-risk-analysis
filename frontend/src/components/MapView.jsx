...
const getLatLng = (item) => {
  ...
  const lat = parseFloat(
    item.lat ?? item.latitude ?? item.lat_centroid ?? item.y,
  );

  const lng = parseFloat(
    item.lng ?? item.longitude ?? item.lng_centroid ?? item.x,
  );
...
function MapView({ filters }) {
  ...
  const hasSelectedProvince = Boolean(
    selectedProv && selectedProv !== "" && selectedProv !== "ทั้งหมด",
  );
  ...
  const fetchHotspots = async () => {
    ...
    try {
      ...
      if (data.success && data.data && data.data.length > 0) {
        ...
        if (hasSelectedProvince) {
          const targetProv = String(selectedProv).trim().toLowerCase();
  ...
  const getRiskInfo = (score, level) => {
    ...
    return { color: "#22c55e", label: "ต่ำ", radius: 8 };
  };

  const currentProvinceName =
    hotspots[0]?.province_name || filters?.province_name || filters?.province;

    const isBangkok = 
        String(currentProvinceName).includes("กรุงเทพ") || 
    ...
    <div className="map-card">
      ...
      <div className="map-container-wrapper">
        ...
        >
          ...
          <MapController
            hotspots={hotspots}
            hasSelectedProvince={hasSelectedProvince}
          />

          {/* Heatmap */}
          {!hasSelectedProvince && <HeatmapLayer points={hotspots} />}
          ...
          {hasSelectedProvince &&
            hotspots.map((item, idx) => {
              ...
              const totalAccidents = Number(item.total_accidents || 0);
              const riskScore = Number(item.risk_score || 0);
              const riskInfo = getRiskInfo(riskScore, item.risk_level);

              console.log("Map item:", item);

              const districtName =
                item.district_name || item.dis_name_th || "ไม่ระบุอำเภอ";
                  ...
                  <Popup>
                    <div className="popup-content">
                      <div className="district-popup">
                        <h4 className="popup-title">{areaLabel} {districtName}</h4>
                        ...
                        <div className="popup-row">
                          ...
                          <strong className="popup-value">
                            {totalAccidents.toLocaleString()} ครั้ง
                          </strong>
                        </div>
